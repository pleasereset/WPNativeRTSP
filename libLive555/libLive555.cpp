...
LibLive555Config CurrentConfig;
std::map<std::string, RTSPClient*> ActiveRTSPClients;
...
void ConfigureRtspLib(const LibLive555Config& configuration)
{
	...
	CurrentConfig.LoggingFunc("libLive555 properly configured");
}

void OpenRtspStream(const char* uri) {
	// Begin by setting up our usage environment:
	TaskScheduler* scheduler = BasicTaskScheduler::createNew();
	UsageEnvironment * env = WPUsageEnvironment::createNew(*scheduler, CurrentConfig.LoggingFunc);

	openURL(*env, "libLive555_WP", uri);
...
void CloseRtspStream(const char* uri)
{
	...
	RTSPClient* targetClient = ActiveRTSPClients[uriString];
	shutdownStream(targetClient);
}
...
RTSPClient* openURL(UsageEnvironment& env, char const* progName, char const* rtspURL) {
	...
	ActiveRTSPClients[rtspURLStr] = rtspClient;
...
void shutdownStream(RTSPClient* rtspClient, int exitCode) {
	...
	// Remove the client from the ActiveRTSPClients map
	std::map<std::string, RTSPClient*>::iterator itElement = ActiveRTSPClients.end();
	for (auto it=ActiveRTSPClients.begin(); it!=ActiveRTSPClients.end(); ++it)
	{
		if(it->second == rtspClient)
		{
			itElement = it;
			break;
		}
	}
	if(itElement != ActiveRTSPClients.end()) ActiveRTSPClients.erase(itElement);
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
	...
	CurrentConfig.DataChunkFunc(fReceiveBuffer, DUMMY_SINK_RECEIVE_BUFFER_SIZE);
